...
class VideoSchedulerService {
  ...
  async initialize() {
    ...
    try {
      ...
      // Sends videos at 12.00 PM NY time
      this.noonJob = cron.schedule('0 12 * * *', async () => {
        console.log('Running 12:00 PM video scheduler check...');
      ...
      // Sends videos at 6.00 PM NY time
      this.eveningJob = cron.schedule('0 18 * * *', async () => {
        console.log('Running 6:00 PM video scheduler check...');
      ...
      console.log('📅 Videos sent to webhook: 12:00 PM (for 12:00 PM publish) and 6:00 PM (for 6:00 PM publish)');
      console.log('⏰ to 10 platforms');
  ...
  async publishScheduledVideo(video) {
    try {
      ...
      console.log(`   Sending at: ${now.format('YYYY-MM-DD hh:mm A')}`);
      ...
      if (webhookResult.success) {
        ...
        console.log(`   Will publish to 10 platforms by ${video.scheduledTime}`);
  ...
  async manualCheck() {
    ...
    // Check if it's time to send for noon videos (12:00 PM - 12:09 PM)
    if (currentHour === 12 && currentMinute >= 0) {
      await this.checkAndPublishVideos('12:00');
    } 
    // Check if it's time to send for evening videos (6:00 PM - 6:09 PM)
  ...
  getStatus() {
    ...
    return {
      ...
      webhookSendTimes: ['12:00 PM (noon)', '6:00 PM (evening)'],
      processingBuffer: '10 minutes',
      platforms: 10,
      cronExpressions: ['0 12 * * * (12:00 PM)', '0 18 * * * (6:00 PM)'],
      description: 'Videos sent to webhook and publish to 10 platforms at scheduled time',
      nextNoonSend: '12:00 PM daily (for 12:00 PM publish)',
      nextEveningSend: '6:00 PM daily (for 6:00 PM publish)'
    };
  }
}
